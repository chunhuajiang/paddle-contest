...
if __name__ == '__main__':
    ...
    try:
        p_zip = zipfile.ZipFile(args.prediction_filename, 'r')
        if not p_zip:
            json_data['errorCode'] = 2
            json_data['errorMsg'] = 'open prediction file failed.'
            print json.dumps(json_data)
            sys.exit(2)
        if len(p_zip.namelist()) != 1:
            json_data['errorCode'] = 3
            json_data['errorMsg'] = 'input zip file should contain only one result file.'
            print json.dumps(json_data)
            sys.exit(3)

        p_zip.extractall()
        args.prediction_filename = p_zip.namelist()[0]
        p_zip.close()
        ...
        print json.dumps(json_data)
        os.remove(args.prediction_filename)
